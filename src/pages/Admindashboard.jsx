...
// Service neve ID alapján (ha nincs serviceName tárólva)
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content modal-content--wide" onClick={(e) => e.stopPropagation()}>
        ...
        <p style={{ color: "hsl(340,30%,50%)", fontSize: "13px", marginBottom: "16px" }}>
          Admin által manuálisan létrehozott foglalás. Konfliktusellenőrzés aktív.
        </p>
...
// Category color based on service prefix
...
const CalendarView = ({ bookings, blockedDates, onEdit, onCancel, onDelete, onAttendance, onAddBooking }) => {
  const now = new Date();
          ...
          {weeks.map((week, wi) => (
            <div key={wi} className="cal-week">
              {week.map(({ date, thisMonth }, di) => {
                ...
                const upcoming   = dayBooks.filter(b => !isCompleted(b));
                ...
                return (
                  ...
                  >
                    ...
                    {isBlocked && (
                      <div className="cal-blocked-tag">Szabadnap</div>
                    )}
                    ...
                    {dayBooks.length > 0 && (
                      <div className="cal-day__bookings">
                        ...
                        {done.length > 0 && (
                          <div className="cal-booking-pill cal-booking-pill--done">
                            +{done.length} lezajlott
                          </div>
                        )}
                        {upcoming.length > 3 && (
                          <div className="cal-booking-pill cal-booking-pill--more">
                            +{upcoming.length - 3} további
                          </div>
                        )}
                      </div>
                    )}
        ...
        {selectedDay && (
          <div className="cal-detail-panel">
            ...
            {blockedSet.has(selectedDay) && (
              <div className="cal-detail-blocked">
                🚫 Ez a nap blokkolva van (szabadnap)
              </div>
            )}
            ...
            <div className="cal-detail-list">
              {selectedBookings.map((b) => {
                ...
                return (
                  <div key={b.id} className={`cal-detail-card${done ? (noshow ? " cal-detail-card--noshow" : " cal-detail-card--done") : ""}`}>
                    <div className="cal-detail-card__stripe" style={{ background: catColor }} />
                    <div className="cal-detail-card__body">
                      <div className="cal-detail-card__top">
                        ...
                        {!done && <span className="att-badge att--upcoming">⏳ Közelgő</span>}
                      </div>
                      ...
                      <div className="cal-detail-actions">
                        {done && (
                          <AttendanceToggle booking={b} onChange={onAttendance} />
                        )}
                        <button className="btn-edit"   title="Szerkesztés"           onClick={() => onEdit(b)}><IoMdCreate /></button>
                        <button className="btn-cancel" title="Visszamondás (email)"  onClick={() => onCancel(b.id)}><IoMdMail /></button>
                        <button className="btn-delete" title="Törlés (email nélkül)" onClick={() => onDelete(b.id)}><IoMdTrash /></button>
                      </div>
                    </div>
                  </div>
                );
      ...
      <div className="modal-content modal-content--wide" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="user-detail-header">
          ...
          <div>
            ...
            {user.banned && (
              <span style={{ marginLeft: "8px", background: "linear-gradient(135deg, #dc2626, #b91c1c)", color: "white", fontSize: "0.75rem", fontWeight: 700, padding: "3px 10px", borderRadius: "999px", letterSpacing: "0.04em" }}>
                🚫 TILTVA
              </span>
            )}
          </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="admin-table-wrapper">
            <table className="admin-table">
              <thead>
                <tr><th>Dátum</th><th>Idő</th><th>Szolgáltatás</th><th>Időtartam</th><th>Részvétel</th><th>Műveletek</th></tr>
              </thead>
              <tbody>
                {filtered.map((b) => (
...
const AdminDashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    return { totalUsers: users.length, upcoming: upcoming.length, completed: completed.length, noshow: noshow.length, attended: attended.length };
  }, [bookings, users]);
                            ...
                            >
                              ...
                              <td className="action-buttons" onClick={(e) => e.stopPropagation()}>
                                ...
                                <button
                                  ...
                                  style={{ width:"30px", height:"30px", borderRadius:"6px", border:"none", cursor: banLoading === user.id ? "not-allowed" : "pointer", fontSize:"14px", display:"inline-flex", alignItems:"center", justifyContent:"center",
                                    background: isBanned ? "linear-gradient(135deg, hsl(140,65%,45%), hsl(140,75%,50%))" : "linear-gradient(135deg, hsl(0,70%,55%), hsl(0,80%,60%))",
                                    color:"white", boxShadow: isBanned ? "0 2px 8px rgba(50,180,80,0.3)" : "0 2px 8px rgba(220,50,50,0.3)", opacity: banLoading === user.id ? 0.6 : 1,
                                  }}
                                >
